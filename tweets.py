...
def get_tweets_csv():
    all = pd.read_csv(user + ".csv")
    tweets_only = all["text"]
    open(user+".txt", "w+")
    tweets_only.to_csv("onlytweets.csv", index = False)
    with open('onlytweets.csv', newline='') as csvfile:
        tweets_txt = open(user + ".txt", "a")
        readCSV = csv.reader(csvfile, delimiter=',')
        count = 0
        for row in readCSV:
            if count == 1 : print(row[0])
            count += 1
            if (row[0])[0] is not '"':
                tweets_txt.write(row[0] + "\n")
